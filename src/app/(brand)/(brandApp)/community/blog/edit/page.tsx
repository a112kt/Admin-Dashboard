...
import { usePost, useEditPost } from '@/features/brand/community/hooks/useCommunityPosts';
...
function BlogEditPage() {
  ...
  useEffect(() => {
    if (post) {
      ...
      setEnableComments(post.commentsEnabled);
      if (post.coverImageUrl) setCoverPreview(post.coverImageUrl);
    }
  }, [post]);
  ...
  const handleSave = async (status: 'published' | 'draft') => {
    ...
    if (!postId) { setError(t('No post selected.')); return; }
    setError('');
    try {
      await editPost({ postId, title, content, status, commentsEnabled: enableComments });
      router.push('/community/blog');
    } catch (err: any) {
      setError(err?.response?.data?.message?.en || t('Failed to update post.'));
    }
  };

  if (!postId) {
    return (
      <PageContainer title={t('Edit Article')} description="Edit blog post">
        <Box sx={{ p: 4, textAlign: 'center' }}>
          <Typography variant="h5">{t('No post selected.')}</Typography>
          <Button component={Link} href="/community/blog" sx={{ mt: 2 }}>{t('Back to Blog')}</Button>
        </Box>
      </PageContainer>
    );
  }
  ...
  return (
    <PageContainer title="Blog" description="Edit blog post">
      <Box
        sx={{
          bgcolor: '#1b2351',
          borderRadius: '20px',
      ...
      <Stack direction="row" spacing={2} justifyContent="flex-end">
        <Button
          ...
          disabled={isPending || isPostLoading}
          sx={{
            fontFamily: '"Onest", sans-serif',
            fontWeight: 500,
            fontSize: '14px',
...
function BlogEditPageWrapper() {
  const { t } = useTranslation();
  return (
    <Suspense fallback={<Box sx={{ p: 4, textAlign: 'center' }}><Typography>{t('Loading...')}</Typography></Box>}>
      <BlogEditPage />
    </Suspense>
  )
}

export default BlogEditPageWrapper
