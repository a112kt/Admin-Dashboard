...
import { useCreateNewChat, useGetBrandOwnerId } from "@/features/brand/chat/hooks/chatApiHooks";
import { getCurrentUserId } from "@/utils/helpers/chatHelpers";
...
const BrandProfilePage = () => {
  ...
  const { data: brandRes, isLoading, isError } = useGetBrandDetails(brandId);
  ...
  const { data: brandOwnerId } = useGetBrandOwnerId(brandId);
  const { mutateAsync: createNewChat, isPending: isChatCreating, isSuccess, isError: isChatError, data: newChatData } = useCreateNewChat();
  ...
  useEffect(() => {
    if (brand && currentUserId) {
      setIsOwner(brand.owner.userId === currentUserId);
    }
  }, [brand, currentUserId]);
  ...
  const handleCoverUpload = async (file: File) => {
    try {
      await uploadCover({ brandId, file });
  ...
  const handleMessage = useCallback(() => {
    if (brandOwnerId?.data?.ownerId) {
      createNewChat(brandOwnerId.data.ownerId);
    }
  }, [brandOwnerId, createNewChat]);

  useEffect(() => {
    if (isSuccess && newChatData?.data) {
      router.push(`/chat?chatId=${encodeURIComponent(newChatData.data)}`);
    }
  }, [isSuccess, newChatData, router]);
  ...
  if (isLoading) {
    return (
      <PageContainer title="Brand Profile">
        <Breadcrumb title="Brand Profile" items={BCrumb} />
  ...
  return (
    <PageContainer title={`${brand.displayName} - Brand Profile`}>
        ...
        <Stack direction="row" spacing={1}>
          ...
          {!isOwner && !isEditing && (
            <Button
              variant="contained"
              color="primary"
              startIcon={isChatCreating ? <CircularProgress size={18} /> : <Icon icon="solar:chat-round-dots-line-duotone" width={18} />}
