...
const VariationCard = ({ variations, setVariations }: {
  ...
  const { data: sizes, isLoading: isLoadingSizes, isSuccess: isSizesSuccess } = useGetSizes();
  const { data: colors, isLoading: isLoadingColors, isSuccess: isColorsSuccess } = useGetColors();
  const [colorsOptions, setColorsOptions] = useState<Option[]>([]);
  const [sizesOptions, setSizesOptions] = useState<Option[]>([]);

  useEffect(() => {
    setVariations([{ id: Date.now(), color: { value: "", label: "", sizes: [{ value: "", label: "", quantity: "" }] } }]);
  }, []);
  ...
  useEffect(() => {
    const takenColors = new Set(
      variations
        // .filter((v) => v.id !== variation.id && v.color.value)
        .map((v) => v.color.value)
    );
    setTakenColors(takenColors);
  }, [variations]);
  const [takenSizes, setTakenSizes] = useState<Record<string, Set<string>>>({

  });
  useEffect(() => {
    const takenSizes: Record<string, Set<string>> = {};

    variations.forEach(variation => {
      if (!takenSizes[variation.color.value]) {
        takenSizes[variation.color.value] = new Set<string>();
      }

      variation.color.sizes.forEach(size => {
        if (size.value) {
          takenSizes[variation.color.value].add(size.value);
        }
      });
    });

    setTakenSizes(takenSizes);
  }, [variations]);
          ...
          <Grid container spacing={2} mb={2} alignItems="center">
            <Grid size={{ xs: 12, lg: 10 }}>
              <CustomSelect
                ...
                displayEmpty
              >
                {!variation.color.value && <MenuItem value="" disabled>Select Color</MenuItem>}
                {
                  colorsOptions.map((option: Option) => (
                    <MenuItem
                      key={option.value}
                      value={String(option.value)}
                      disabled={takenColors.has(String(option.value))}
                    >
                      {option.label}
                    </MenuItem>
                  ))
            ...
            <Grid container spacing={2} mb={2} alignItems="center" key={sizeIndex}>
              <Grid size={{ xs: 12, lg: 4 }}>
                <CustomSelect
                  ...
                  displayEmpty
                >
                  {!s.value && <MenuItem value="" disabled>Select Size</MenuItem>}
                  {
                    sizesOptions.map((option: Option) => (
                      <MenuItem key={option.value} value={String(option.value)} disabled={takenSizes[variation.color.value]?.has(String(option.value))}>
                        {option.label}
                      </MenuItem>
                    ))}
                </CustomSelect>
