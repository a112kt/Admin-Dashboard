...
import { BlogPostType, BlogContext, BlogContextProps } from "@/context/blogContext/BlogContext";
      ...
      {post ? (
        ...
        >
          <CoverImgBg className="hoverCard">
            <>
              <Typography component={Link} href={`/community/blogs/detail/${linkTo}`}>
                <CoverBox
                  sx={{
                ...
                >
                  <Box>
                    <Stack direction="row" alignItems="center">
                      <Link href={`/brand-profile/${author?.brandId}`} style={{ textDecoration: 'none' }}>
                        <Tooltip title={author?.name} placement="top">
                          <Avatar
                            aria-label="recipe"
                            src={author?.avatar}
                            sx={{ transition: 'opacity 0.2s', '&:hover': { opacity: 0.8 } }}
                          ></Avatar>
                        </Tooltip>
                      </Link>
                      <Link href={`/brand-profile/${author?.brandId}`} style={{ textDecoration: 'none' }}>
                        <Chip
                          sx={{ marginLeft: "auto", cursor: 'pointer', transition: 'opacity 0.2s', '&:hover': { opacity: 0.8 } }}
                          label={category}
                          size="small"
                          color="primary"
                        ></Chip>
                  ...
                  <Box>
                    <Box my={3}>
                      <Typography
                        ...
                        component={Link}
                        href={`/community/blogs/detail/${linkTo}`}
                      >
                        {title}
                      </Typography>
                    </Box>
